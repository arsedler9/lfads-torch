import numpy as np
import glob, os, h5py, pdb, pickle
...
def plot_firing_rates(firing_rates, conditionIDs, title, neurons = None, single_trial=False, n_single_trials=10, lims=None, scale=None): 
    '''
    ...
    n_plots = int(np.ceil(n_neurons / 20.))
    ...
    if single_trial:
        for c in range(n_conds): 
            ...
            for n in range(n_neurons): 
                axs[n].plot(cond_trials[n, :, 0:n_single_trials] / scale, color=clr, linewidth=0.75, alpha=0.6)
...
def get_lfads_output(output_files,binwidth):
    ...
    for file in output_files:
        ...
        factors = np.zeros((h5_out['train_inds'].shape[0] + h5_out['valid_inds'].shape[0], h5_out['train_factors'].shape[1], h5_out['train_factors'].shape[2]))
        rates = np.zeros((h5_out['train_inds'].shape[0] + h5_out['valid_inds'].shape[0], h5_out['train_output_params'].shape[1], h5_out['train_output_params'].shape[2]))
...
def run_five_fold_cv_decoding(neural_data, behavioral_data, n_lag=0): 
    if n_lag > 0: 
        ...
        stacked_days_neural_data = np.concatenate([neural_data[k][:, :-n_lag, :] for k in sorted(neural_data.keys())], axis=-1)
        # trim the beginning of the predictee 
        stacked_days_behavior = np.concatenate([behavioral_data[k][:, n_lag:, :] for k in sorted(behavioral_data.keys())], axis=-1)
    else: 
        stacked_days_neural_data = np.concatenate([neural_data[k] for k in sorted(neural_data.keys())], axis=-1)
        stacked_days_behavior = np.concatenate([behavioral_data[k] for k in sorted(behavioral_data.keys())], axis=-1)
